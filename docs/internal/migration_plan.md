...
> 说明：本文取代旧版「GUI→CoverService」迁移叙述；该历史计划见 `cover_skill/migration_plan.md`（P2 已完成）。
...
- [x] `migration_plan.md`  
- [ ] （实现期）核对磁盘文件存在：`G_16000.pth`、`G_27200.pth`、`config_example.json`、`config1.json`
...
3. 在 **SVC 推理取路径处**（推荐 `SVCService` so-vits 分支或 Adapter 预解析）调用 Registry：  
   - 输入仍为现有 `model_name`  
   - `G_16000` → example_voice_b 资产 → `config_example.json`  
...
- 选 `G_16000` 实际加载 `G_16000.pth` + `config_example.json`  
- GUI 仍能生成翻唱；现有测试通过  
...
- 选 ExampleVoiceB → 实际 G_16000 + config_example  
...
| 风险 | 缓解 |
|------|------|
| `config_example` encoder 与 checkpoint 不匹配 | Phase 0 用短音频真推理验收 example_voice_b / example_voice 各一次 |
...
2. `speaker=example_voice_b` → `G_16000.pth` + `config_example.json`  
